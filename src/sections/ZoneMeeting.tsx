import React from 'react';

function ZoneMeeting() {
    return (
      <div>
        <div className="main-content main-content-table">
          <h2 className="header">SPS Zone 3 Meeting at Lycoming College</h2>
          <span id="top-span">SPS Lycoming is delighted to host the 2022-23 SPS Zone 3 Meeting and we are looking forward to hosting your chapters! The meeting will be held on Saturday, February 25th, from 8:00am to 9:00pm. We encourage folks to arrive Friday, February 24th to settle in and enjoy light refreshments and a fun activity. There is no registration fee. Please visit <a href="https://forms.office.com/r/arHBSddDNb">this link</a> to RSVP, we ask that all attendees RSVP by February 1st, 2023. <b>Late registrations will be accepted until February 18th, 2023.</b></span>

          <span>We've included directions to Lycoming College below, along with a campus map and housing information. Our tentative itinerary is listed below and will be finalized before the registration deadline. If you have any questions about the meeting or logistics, please email the Lycoming SPS Executive Board at <a href="mailto:sps@lycoming.edu?subject=SPS Zone Meeting">sps@lycoming.edu.</a></span>

          <table className="”sortable”" id="striped" style={{marginTop: 55 + 'px'}}>
            <thead>
              <tr>
                <th>Day</th>
                <th>Time</th>
                <th className="sorttable_nosort">Event</th>
                <th>Location</th>
              </tr>
            </thead>
            <tbody className="output">
            <tr>
            </tr>
            <tr>
              <td>Friday</td>
              <td>7:00pm-7:30pm</td>
              <td>Registration</td>
              <td>Academic Center Lobby</td>
            </tr>
            <tr>
              <td>Friday</td>
              <td>7:30pm-9:00pm</td>
              <td>Game Night</td>
              <td>Academic Center Lobby</td>
            </tr>
            <tr>
              <td>Saturday</td>
              <td>9:00am-10:00am</td>
              <td>Registration & Breakfast</td>
              <td>Academic Center Lobby</td>
            </tr>
            <tr>
              <td>Saturday</td>
              <td>10:00am-10:30am</td>
              <td>Opening/History of SPS</td>
              <td>Heim G-11</td>
            </tr>
            <tr>
              <td>Saturday</td>
              <td>10:30am-11:30am</td>
              <td>Plenary Speaker: <a href="https://umdphysics.umd.edu/people/faculty/current/item/167-gatess.html"> Dr. Jim Gates</a></td>
              <td>Heim G-11</td>
            </tr>
            <tr>
              <td>Saturday</td>
              <td>11:30am-12:30pm</td>
              <td>Lunch</td>
              <td>Academic Center Lobby</td>
            </tr>
            <tr>
              <td>Saturday</td>
              <td>12:30pm-1:30pm</td>
              <td>Workshop</td>
              <td>Lynn Classrooms</td>
            </tr>
            <tr>
              <td>Saturday</td>
              <td>1:30pm-2:30pm</td>
              <td>Career Scientists Panel</td>
              <td>Heim G-09</td>
            </tr>
            <tr>
              <td>Saturday</td>
              <td>2:30pm-3:00pm</td>
              <td>Coffee Break</td>
              <td>Heim Lobby</td>
            </tr>
            <tr>
              <td>Saturday</td>
              <td>3:00pm-4:00pm</td>
              <td>Research Talks</td>
              <td>Heim G-09</td>
            </tr>
            <tr>
              <td>Saturday</td>
              <td>4:00pm-5:00pm</td>
              <td>Poster Sessions</td>
              <td>Heim Lobby</td>
            </tr>
            <tr>
              <td>Saturday</td>
              <td>5:00pm-6:00pm</td>
              <td>Dinner</td>
              <td>Academic Center Lobby</td>
            </tr>
            <tr>
              <td>Saturday</td>
              <td>6:00pm-7:00pm</td>
              <td>Planetarium Show</td>
              <td>Detwiler Planetarium</td>
            </tr>
            </tbody>
            <tfoot></tfoot></table>

            <span><i>There will be a scavenger hunt held throughout the day on Saturday. Meals on Saturday are provided.</i></span>

        </div>
      </div>
    );
  }
  export default ZoneMeeting;